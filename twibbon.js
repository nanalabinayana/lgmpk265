/* twibbon.js – PK265 (rev settings-card + left drawer) — JSON mode
 ...
 * - DATA dari JSON atau window.DATA (data_peserta_pk265.json / data_peserta.js)
 * - Degree/Negara statis + icon bendera
 * - Nama: highlight valid/invalid (tanpa badge teks) + AUTO WRAP 2 baris
...
/* ================= KONSTANTA ================= */
const MAX_NAME_CHARS   = 25; // buat saran singkatan
const LIMITS = { nameWithSpaces: 25, univWithSpaces: 32, prodiNoSpaces: 34 };

/* ==== GANTI: sumber data dari JSON ==== */
/** Ubah ke URL/path yang valid di server kamu */
const DATA_JSON_URL     = "data_peserta_pk265.json";
const FRAME_SVG         = "frame.svg";
const FRAME_PNG         = "frame.png";
const TEXT_SVG_TEMPLATE = "text-template.svg";
...
/* === helper untuk wrap nama 2 baris di UI === */
function escapeHTML(s){ return String(s).replace(/[&<>"']/g, m=>({ "&":"&amp;","<":"&lt;",">":"&gt;","\"":"&quot;","'":"&#39;" }[m])); }
function wrapNameForBox(name, limit=LIMITS.nameWithSpaces){
  const v = String(name||"").trim();
  if (!v) return "—";
  if (v.length <= limit) return escapeHTML(v);
...
/* ===== INIT ===== */
(async function init(){
  // Muat data jika perlu (JSON dulu, atau window.DATA jika sudah ada)
  if (!DATA.length){
    try{
      const text = await fetchText(DATA_JSON_URL);
      let raw = JSON.parse(text);
      // Terima beberapa bentuk: array langsung, atau objek dengan key umum
      let arr = Array.isArray(raw)
        ? raw
        : (raw?.data || raw?.participants || raw?.items || []);
      ...
      console.warn("Gagal memuat JSON:", e.message);
      DATA = [];
    }
  }else{
...
async function chooseRecord(rec){
  ...
  // --- KIRIM PILIHAN KE PARENT (index-combined) ---
  window.parent?.postMessage({
    type: 'twibbon:selected',
...
rowNameSelect?.addEventListener("change", async ()=>{
  ...
  // kabari parent lagi setelah nama diedit
  window.parent?.postMessage({
    type: 'twibbon:selected',
    displayName: getDisplayName(),
    degree: selected?.["Jenis Gelar"] || selected?.["Jenjang"] || selected?.["Degree"] || "",
    record: selected
  }, '*');
});
